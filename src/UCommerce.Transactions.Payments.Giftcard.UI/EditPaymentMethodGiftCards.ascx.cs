			                   ...
			                   select new
								{
									...
									SpentBalance = new Money(groupedByCurrency.Sum(x => x.AmountUsed), groupedByCurrency.Key.ISOCode).Value.ToString("N"),
									OutstandingBalance = new Money(groupedByCurrency.Sum(x => x.Amount) - groupedByCurrency.Sum(x => x.AmountUsed), groupedByCurrency.Key.ISOCode).Value.ToString("N"),
									TotalBalance = new Money(groupedByCurrency.Sum(x => x.Amount), groupedByCurrency.Key.ISOCode).Value.ToString("N")
								};
